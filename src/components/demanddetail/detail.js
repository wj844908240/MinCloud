...
class Detail extends Component  {
  ...
  componentWillMount(){
    ...
    TOOLS.get(`supplyservice/service/demand/${id}/follow/`).then(res => {
      if(res.status === 200){
        this.setState({
    ...
    TOOLS.get(`supplyservice/product/demand/${id}/follow/`).then(res => {
      if(res.status === 200){
        this.setState({
  ...
  productCern(id,type){
    if(type==="service"){
      ...
      //请求供关注
      TOOLS.put( `supplyservice/service/demand/${id}/follow/`,{
      ...
      //请求供关注
      TOOLS.put(`supplyservice/product/demand/${id}/follow/`,{
        is_followed : followed
      }).then(res => {
        if(res.status === 200){
          this.setState({
    ...
    const {Resource,type}=this.props
        ...
        <div>
          ...
          <Carousel autoplay>
            { type === "server" ? Resource.service_demand_pics && Resource.service_demand_pics.length > 0 &&Resource.service_demand_pics.map(item=>{
                return <div key={item.id}><img src={item.pic}/></div>
              }) : Resource.product_demand_pics && Resource.product_demand_pics.length > 0 &&Resource.product_demand_pics.map(item=>{
                return <div key={item.id}><img src={item.pic}/></div>
              })
            }
          </Carousel>
          ...
          <div className="information">
              ...
              <ul>
                ...
                </li>
                {
                   Resource.owner_type === "enterprise" ? <li className="wh">
                      <div><b>联系电话：</b><span>{Resource.enterprise.mobile}</span></div>
            ...
            <tbody>
            {
              type === "server" ? Resource.service_demand_materials && Resource.service_demand_materials.length > 0 &&Resource.service_demand_materials.map((item,index)=>{
                return <tr key={index}> 
                  <td>{item.name === void(0)?"":item.name}</td>
                  <td>{item.suffix}</td>
                  <td>{item.size}M</td>
                  <td><a href={item.material}>下载</a></td>
              </tr>}) : Resource.product_demand_materials && Resource.product_demand_materials.length > 0 &&Resource.product_demand_materials.map((item,index)=>{
                return <tr key={index}> 
                  <td>{item.name === void(0)?"":item.name}</td>
                  <td>{item.suffix}</td>
                  <td>{item.size}M</td>
