...
class List extends Component  {
  GoLink(id,url){
    this.props.history.push(`${url}?id=${id}&type=demand`);
    ...
    const soureList = (item,index) =>{
      return(
          <li onClick={e=>this.GoLink(item.id,Addr)} key={index+1}>
            <h6>
              {
                type === "server" ? <img src={item.service_demand_logos.length===0 ?require("../../img/u689.png") : item.service_demand_logos[0].logo }/>
                :<img src={item.product_demand_logos.length===0 ?require("../../img/u689.png") : item.product_demand_logos[0].logo }/>
              }
            </h6>
